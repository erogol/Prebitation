...
def find_cluster_centers(timeseries, k):
    ...
    """
    k_means = KMeans(n_clusters=100, init='k-means++', n_init=10, max_iter=300, tol=1e-6,
...
def predict_dpi(x, s):
    """Predict the average price change delta_p_i, 1 <= i <= 3
...
def linear_regression_vars(prices, v_bid, v_ask, s1, s2, s3):
    """Use the second time period to generate the independent and dependent variables
       in the linear regression model delta_p = w0 + w1 * delta_p1 + w2 * delta_p2 + w3 * delta_p3 + w4 * r.
    ...
    Returns:
        ...
        row represents [delta_p1, delta_p2, delta_p3, r]. Y is a numpy array of floats and
        each array element represents delta_p.
    """
...
def predict_dps(prices, v_bid, v_ask, s1, s2, s3, w):
    """Predict average price changes (final estimations delta_p) over the third
       time period.
...
def evaluate_performance(prices, dps, t, step):
    ...
    for i in range(720, len(prices) - 1, step):
        ...
        if dps[i - 720] < -t and position >= 0:
            position -= 1
            bank_balance += prices[i]
        # computer real price shift
        real_dps.append(last_price - prices[i])
        last_price = prices[i]
